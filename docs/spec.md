...
用户上传的文件 + agent 生成的中间文件。agent 可直接读写。拖进 chat 可作为显性上下文。
...
给 agent 的可复用操作手册。描述"如何做 X"，含文档和代码。Mira runtime 原生加载。

### Dispatch

入站消息通道。让 agent 可以通过飞书 bot / webhook 被外部触发。外部消息进来 → 查找或创建 Session → agent 回复 → 反向推送。
