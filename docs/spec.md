...
- 每个 Catwork 有不可变 id（创建时生成）。name 是显示名，可随时修改。内部绑定（Session、Trace 等）全部走 id，不走 name
...
修改 Catwork 的显示名。id 不变，Session 绑定不受影响。
...
1. **绑定**：创建时通过不可变 id 写入所属 Catwork，此后不可变更。Rename Catwork 不影响 Session 归属
...
Task 本质是预设的 prompt + trigger + 可选的预设上下文。`/name` 是人调 agent 的快捷方式。Template 可以预置 manual task 作为引导项。点击 Task 条目 → 在 Artifacts 面板打开编辑器。
...
外部消息进来 → 按 bot 绑定关系找到目标 Catwork → 查找或创建 Session → agent 回复 → 反向推送。点击 Dispatch 条目 → 在 Artifacts 面板打开编辑器。
...
| 2026-04-17 | Catwork 加不可变 id；Session 绑定改为 id；加 Rename；去掉 status/subagents/sandbox UI；Dispatch 两种模式；Task/Dispatch 编辑走 Artifacts；Skill 压缩包上传；Template = YAML + files 打包。 |
